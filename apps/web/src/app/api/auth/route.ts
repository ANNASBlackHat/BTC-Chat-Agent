...
export async function POST(req: Request) {
  try {
    ...
    const response = NextResponse.json({
      ...
      // Mobile clients (Expo) have no reliable cookie jar; they persist this
      // token themselves (e.g. expo-secure-store) and send it as a Bearer header.
      token: sessionToken,
    });
    ...
    response.cookies.set('session_token', sessionToken, {
      ...
      sameSite: 'lax',
      path: '/',
      maxAge: 60 * 60 * 24 * 30, // 30 days session lifespan
    });
